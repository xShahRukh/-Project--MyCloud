...
//		if($this->_2d)
	...
	function get_point($series,$row)
	{
		...
		//print_r($row);
		$str="<point id=\"" . $strLabelFormat . "\" name=\"" . $strLabelFormat . "\" y=\"". $this->chart_xmlencode(str_replace(",",".",$strDataSeries)+0). "\">";
...
class Chart_Combined extends Chart
{
	...
	function get_point($series,$row)
	{
		...
		if($this->table_type!="db" || !count($this->chrt_array['customLabels']))
			...
			$strDataSeries = $row[$this->chrt_array['customLabels'][$this->arrDataSeries[$series]]];
		$str="<point id=\"" . $strLabelFormat . "\" name=\"" . $strLabelFormat . "\" y=\"". ($this->chart_xmlencode($strDataSeries)+0). "\">";
		$str.="<tooltip enabled=\"True\"><format>".$this->tooltipFormat($series,$row)."</format></tooltip>"."\n";
		$str.= $this->getActions( $row );
		$str.="</point>"."\n";
		return $str;
...
//		}
	...
	function get_point($series,$row)
	{
		...
		$str="<point ".$id_name." y=\"". $this->chart_xmlencode($strDataSeries+0). "\" size=\"" . $this->chart_xmlencode(str_replace(",",".",$row[$this->arrDataSize[$series]])+0). "\">";
		$str.="<tooltip enabled=\"True\"><format>".$this->tooltipFormat($series,$row)."</format></tooltip>"."\n";
		$str.= $this->getActions( $row );
		$str.="</point>"."\n";
		return $str;
